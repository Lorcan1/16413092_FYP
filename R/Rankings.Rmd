...
#### sampled results ####
set.seed(1)
df <- read.csv(paste0(getwd(), "/fairCombosOrig.csv"), stringsAsFactors = T)
...
ggplot(combined_new, aes(x=Scenario,y=FairRankAll, fill=Scenario)) + geom_boxplot() + ggtitle("Mean Rank: Fair")
...
ggplot(combined_new, aes(x=Scenario,y=EqualWeightRank, fill=Scenario)) + geom_violin() + ggtitle("Mean Weighted Rank: Perf + Fair + Runtime")
ggsave("BoxplotMeanWeightedRankAllViolin.png", width = 20, height = 14, units = "cm")

ggplot(combined_new, aes(x=Scenario,y=PerfFairEqualRank, fill=Scenario)) + geom_violin() + ggtitle("Mean Weighted Rank: Perf + Fair")
ggsave("BoxplotMeanWeightedRankPerfFairViolin.png", width = 20, height = 14, units = "cm")

ggplot(combined_new, aes(x=Scenario,y=PerfRank, fill=Scenario)) + geom_violin() + ggtitle("Mean Rank: Perf")
ggsave("BoxplotMeanWeightedRankPerfViolin.png", width = 20, height = 14, units = "cm")

ggplot(combined_new, aes(x=Scenario,y=FairRankAll, fill=Scenario)) + geom_violin() + ggtitle("Mean Rank: Fair")
...
ggplot(combined_new, aes(x=Scenario,y=RuntimeRank, fill=Scenario)) + geom_violin() + ggtitle("Mean Rank: Runtime")
...
ggplot(combined_new_temp, aes(x = PerfFairEqualRank, y = RuntimeRank)) + geom_point(aes(shape = factor(Type))) +   geom_point(data = skyl, size = 3, aes(color=Scenario)) + plot_front(combined_new, prefs)  + geom_step(data = skyl,direction = "vh") + xlab("Equally Weighted Fairness and Performance Rank") + ylab("Runtime Rank") + labs(shape="Combination")
...
prefs_2 <- low(PerfRank) * low(FairRankAll)
...
ggplot(combined_new_temp, aes(x = PerfRank, y = FairRankAll)) + geom_point(aes(shape = Type)) + plot_front(combined_new, prefs_2) +   geom_point(data = skyl_2, size = 3, aes(color=Scenario)) + geom_step(data = skyl_2,direction = "vh")+ xlab("Performance Rank") + ylab("Fairness Rank") + labs(shape="Combination")
ggsave("FairVsPerf.png", width = 15, height = 10, units = "cm")

# example 3: individual fairness vs group fairness
prefs_3 <- low(FairRankIndiv) * low(FairRankGroup)
...
ggplot(combined_new_temp, aes(x = FairRankIndiv, y = FairRankGroup)) + geom_point(aes(shape = Type)) + plot_front(combined_new, prefs_3) +   geom_point(data = skyl_3, size = 3, aes(color=Scenario))  + geom_step(data = skyl_3,direction = "vh")+ xlab("Individual Fairness Rank") + ylab("Group Fairness Rank") + labs(shape="Combination")
...
labels <- paste0(combined_new$PerfFairEqualRank, "\n", combined_new$RuntimeRank)
...
combined <- deriveEqualRank(aggregateResults(df), weightAcc,weightPrec,weightRec,weightSP,weightDI,weightTI,weightAO,weightEO,weightInd,weightGroup,weightFair,weightPerf,weightRun)
combined <- combined[order(combined$PerfFairEqualRank), ]
...
ggplot(combined_new_temp, aes(x = FairRankGroup, y = FairRankIndiv)) + geom_point(aes(shape = Type)) +   geom_point(data = skyl, size = 3, aes(color=Scenario)) + plot_front(combined_new, prefs)  + geom_step(data = skyl,direction = "vh") + xlab("Group Fairness Rank") + ylab("Individul Fairness Rank") + labs(shape="Combination")
